...
function Icon({ name, size = 50 }: { name: string; size?: number }) {
  ...
  switch (name) {
    ...
    case "whatsapp":
      // Оригинальный логотип WhatsApp: зелёный круг + белая трубка.
      // viewBox 0..50 — path'ы в этих координатах (была ошибка: 0..100,
      // значок уходил в угол и выглядел белым кружком).
      return (
        <svg width={size} height={size} viewBox="0 0 50 50" aria-hidden="true">
          <path d={WA_CIRCLE_D} fill="#27D061" />
          <path d={WA_TUBE_D} fill="#fff" />
        </svg>
      );
    default:
      return null;
