...
> ⚠️ **Порты (2026-08):** порт **3000 занят панелью Codent** (`codent/web`) — сайт Anver запускается на **3111**: `npm run start -- -p 3111` (production) или `npm run dev -- -p 3111` (разработка). Быстрый запуск — `start-site.cmd` (сборка + старт + открытие браузера; с аргументом `dev` — dev-режим).
...
### Уведомления в Telegram о заказах (2026-08)
- [x] Бот @zakaziks6_bot (токен в `.env` → `TELEGRAM_BOT_TOKEN`), группа-получатель (`TELEGRAM_CHAT_ID=-5477386592`)
- [x] `lib/telegram.ts`: `sendTelegramMessage()` (Bot API sendMessage, HTML, экранирование, таймаут 8с, без падений если не настроено), `formatOrderMessage()` — формат уведомления (заказ, имя, контакт, оплата, комментарий, состав с опциями/ценами, итого, язык)
- [x] `POST /api/order` шлёт уведомление в Telegram после успешной записи в БД; ошибка отправки не роняет приём заказа
- [x] Проверено живым тестом: тестовое сообщение-заказ ушло в группу (ok:true)
- [ ] По желанию: те же уведомления для форм обратной связи (feedback/контакты) — вызов `sendTelegramMessage` в `app/api/feedback`
...
### Сделано позже (итерация 4, 2026-08)
- [x] **Селектор расцветок на карточке товара** (`ProductCard.tsx` + `ProductCard.module.css`): под ценой — миниатюры расцветок (кружки 42px с фото ткани, нарезка gallery по `json_options` как в модалке), клик переключает главное фото на фото выбранной расцветки (hover — второе фото расцветки), выбранный цвет уходит в корзину (`addItem(product, color)`); при 0–1 расцветке селектор скрыт, цвет не передаётся; перевод «Расцветка»/«Culoare» в `i18n.ts`
- [x] **Раздел «Кухня»** (`/kitchen`, `lib/kitchen.ts`, `components/KitchenPage.tsx`): скатерти (4 карточки = коллекции Лаванда/Прованс/Шато/Лимонный сад), у каждой 2 фото — `gallery[0]` чистая скатерть (карточка), `gallery[1]` сервированная композиция (hover); фото в `/public/kitchen/`; меню «Кухня» → `/kitchen`; переводы (ru+ro)
- [x] **Выбор размера скатерти с обновлением цены + размер в заказе/Telegram**: тип `Product.sizeOptions` (`{label, price}` — 4 размера: 1,20×1,80/1,80×2,20/Ø1,80/1,80×3,00 → 300/400/350/500 лей); на карточке размер НЕ показывается (только в модалке по клику); в `ProductModal` — селектор размера, цена обновляется при выборе, `addItem(product, color, qty, size)`; корзина хранит `size` с ценой размера (`CartContext.addItem` учитывает `sizeOptions`), заказ (`options.size`) уходит в Telegram как «Размер» (расширены `optionTitleRo` и `modal.size` i18n ru/ro); длинное описание в модалке (`lib/kitchen.ts COMMON_TEXT`)

### Сделано позже (итерация 5 — ресёрч + P0 + отзывы, 2026-08-17)
- [x] **Исследование ниши**: 4 параллельных агента → `research/home-textile-competitors.md` (40 KB), `research/anver-products.md` (16 KB), `research/anver-delivery-logistics.md` (24 KB), `anver-react/research/anver-design-ux.md` (~20 KB). Проанализированы 15+ конкурентов (Молдова/Румыния/EU/US), карточка товара, дизайн/UX, доставка/оплата/маркетинг.
- [x] **TopBanner**: дедлайн обновлён с 08.11.2025 → 30.11.2026 (осень 2026)
- [x] **Рейтинг на hero**: `★★★★★ 800+ довольных клиентов` под CTA с fade-in анимацией (заголовок → описание → кнопка → рейтинг с задержкой 0.6с)
- [x] **Кухня на главной**: секция `anv-kitchen-cta` с CTA «Выбрать комплект» → `/kitchen`, фон `#fcf7f0`, между каталогами и блоком «О нас»
- [x] **Аккордеон в модалке товара** (`ProductModal.tsx`): 3 секции — Состав (100% хлопок), Уход (стирка 40°C/глажка/отбеливание), Доставка (1-3 дня FanCourier, бесплатно от 1000 лей, возврат 14 дней)
- [x] **Таблица размеров** (`components/SizeTable.tsx`): попап с 4 размерами (1.5сп/двусп/евро/семейный), RU+RO, кнопка 📏 в селекторе размера
- [x] **Sticky add-to-cart на мобильных**: фиксированный нижний бар (≤768px) с названием, ценой и кнопкой «В корзину»
- [x] **Гарантии под кнопкой** «В корзину»: «Бесплатно от 1000 лей / Возврат 14 дней / Только 100% хлопок» (текст, без эмодзи)
- [x] **Система отзывов** (агент): таблица `reviews` в Supabase (миграция `supabase/reviews-schema.sql`), API POST/GET `/api/reviews` + админ-модерация `/api/admin/reviews`, страница `/reviews`, компонент `ReviewsBlock` для главной/карточки, ссылка в Header/Footer/MobileMenu, Telegram-уведомления о новых отзывах и feedback
- [x] **Удалены все эмодзи** из UI: 🧵👕🚚🔄✅📏🙏📷 убраны из `ProductModal.tsx`, `SizeTable.tsx`, `HomePage.tsx`, `ReviewsPage.tsx`, `admin/page.tsx`, `lib/i18n.ts` (словари RU/RO — гарантии без эмодзи)
- [x] **Кухня: page title почищен**: убран `<h1>` со страницы `/kitchen`, хлебные крошки используют `nav.kitchen`
- [x] **Кухня: hover-фото исправлены**: переставлены файлы `kitchen-3-hover.png` ↔ `kitchen-4-hover.png` (Шато и Лимонный сад были перепутаны)
- [x] **Страница сравнения фото**: `public/kitchen-compare.html` — все 8 фото в столбик для визуальной проверки
